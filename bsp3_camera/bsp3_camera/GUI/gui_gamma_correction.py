...
from evc_gamma_correction import (  # noqa: E402
...
def gui_gamma_correction(img: np.ndarray) -> np.ndarray:
...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self, img: np.ndarray, parent=None):
        ...
        self._canvas_lut.setSizePolicy(
    ...
    def change_gamma_slider(self, value: int):
    ...
    def change_gamma_input(self, gamma: float):
        """Refresh image with changed gamma value"""
        self.spin_used = True
    ...
    def keep_color_balance_clicked(self, button: QCheckBox):
    ...
    def compute_gamma_corrected_image(self, img: np.ndarray, gamma: float):
        if self.color_balance:
            ...
            self.img_corrected = evc_reconstruct(brightness_corrected, chromaticity)
        else:
