...
from evc_compute_binary import evc_compute_binary  # noqa: E402
from evc_histogram_clipping import (  # noqa: E402
    evc_clip_histogram,
    evc_transform_histogram,
)
from GUI.gui_shared import ClickableImageLabel  # noqa: E402
from matplotlib.backends.backend_qtagg import FigureCanvas  # noqa: E402 # type: ignore
from matplotlib.backends.qt_compat import QtWidgets  # noqa: E402 # type: ignore
...
def gui_gamma_histogram_clipping(img: np.ndarray) -> tuple[float, float]:
...
class ApplicationWindow(QtWidgets.QMainWindow):
    def __init__(self, img: np.ndarray, parent=None):
        ...
        self.selectionDisableList = (
            self.btn_new_selection,
            self.btn_ok,
            self.btn_reset,
            self.low_spin,
            self.high_spin,
    ...
    def refresh_img(self):
        self._img_label_main.set_image(
            np.clip(self.img_corrected * 255, 0, 255).astype(np.uint8)
        )

    def refresh_img_low(self):
        self._img_label_low.set_image(
            np.clip(self.img_low * 255, 0, 255).astype(np.uint8)
        )

    def refresh_img_high(self):
        self._img_label_high.set_image(
            np.clip(self.img_high * 255, 0, 255).astype(np.uint8)
        )

    def change_low_input(self, value: float):
        """Refresh low image"""
        self.low = value
        top = 0
    ...
    def change_high_input(self, value: float):
        """Refresh high image"""
        self.high = value
        top = 1
    ...
    def refreshHistogram(self, img: np.ndarray):
    ...
    def on_clipping_changed(self):
        self.img_corrected = evc_transform_histogram(
            self.img, self.low_spin.value(), self.high_spin.value()
        )
    ...
    def reset(self):
        ...
        self.low = 0
        self.high = 1
        self.new_selection_stage = (
            -1
        )  # avoid double update, not the nicest way, but it works
        self.low_spin.setValue(self.low)
        self.high_spin.setValue(self.high)
        self.new_selection_stage = 0
        self.on_clipping_changed()
...
if __name__ == "__main__":
    ...
    img = np.array(Image.open(".\\bsp3_camera\\GUI\\weiss.jpg")).astype("float") / 255
